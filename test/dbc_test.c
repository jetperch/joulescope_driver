...
static void test_true(void **state) {
    (void) state;
    JSDRV_DBC_TRUE(1);
    expect_assert_failure(JSDRV_DBC_TRUE(0));
}

static void test_false(void **state) {
    (void) state;
    JSDRV_DBC_FALSE(0);
    expect_assert_failure(JSDRV_DBC_FALSE(1));
}
...
static void test_not_null(void **state) {
    ...
    int x = 0;
    int * p = &x;
    JSDRV_DBC_NOT_NULL(p);
    expect_assert_failure(JSDRV_DBC_NOT_NULL(0));
}

static void test_equal(void **state) {
    (void) state;
    JSDRV_DBC_EQUAL(42, 42);
    expect_assert_failure(JSDRV_DBC_EQUAL(1, 2));
}

static void test_gte_zero(void **state) {
    (void) state;
    JSDRV_DBC_GTE_ZERO(0);
    JSDRV_DBC_GTE_ZERO(100);
    expect_assert_failure(JSDRV_DBC_GTE_ZERO(-1));
}

static void test_gt_zero(void **state) {
    (void) state;
    JSDRV_DBC_GT_ZERO(1);
    JSDRV_DBC_GT_ZERO(100);
    expect_assert_failure(JSDRV_DBC_GT_ZERO(0));
}

static void test_lte_zero(void **state) {
    (void) state;
    JSDRV_DBC_LTE_ZERO(0);
    JSDRV_DBC_LTE_ZERO(-100);
    expect_assert_failure(JSDRV_DBC_LTE_ZERO(1));
}

static void test_lt_zero(void **state) {
    (void) state;
    JSDRV_DBC_LT_ZERO(-1);
    JSDRV_DBC_LT_ZERO(-100);
    expect_assert_failure(JSDRV_DBC_LT_ZERO(0));
}

static void test_range_int(void **state) {
    (void) state;
    JSDRV_DBC_RANGE_INT(-10, -10, 20);
    JSDRV_DBC_RANGE_INT(20, -10, 20);
    expect_assert_failure(JSDRV_DBC_RANGE_INT(-11, -10, 20));
    expect_assert_failure(JSDRV_DBC_RANGE_INT(21, -10, 20));
}
