...
void jsdrvp_backend_send(struct jsdrv_context_s * context, struct jsdrvp_msg_s * msg) {
    ...
    } else if (0 == strcmp(JSDRV_BUFFER_MSG_ACTION_LIST, msg->topic)) {
        const size_t list_length = msg->value.size;
        const uint8_t * list_buffers = msg->value.value.bin;
        check_expected(list_length);
        check_expected_ptr(list_buffers);
    } else {
        // ???
...
#define expect_list(ex_list, ex_len) \
    expect_value(jsdrvp_backend_send, list_length, ex_len); \
    expect_memory(jsdrvp_backend_send, list_buffers, ex_list, ex_len)
...
struct jsdrv_context_s * initialize() {
    ...
    expect_meta(JSDRV_BUFFER_MSG_ACTION_ADD "$");
    expect_meta(JSDRV_BUFFER_MSG_ACTION_REMOVE "$");
    expect_meta(JSDRV_BUFFER_MSG_ACTION_LIST "$");
    expect_list(ex_list_buffer, sizeof(ex_list_buffer));
...
int32_t publish(struct jsdrv_context_s * context, struct jsdrvp_msg_s * msg) {
    ...
    jsdrv_list_foreach(&context->subscribers, item) {
        struct sub_s *sub = JSDRV_CONTAINER_OF(item, struct sub_s, item);
        if (0 == strcmp(msg->topic, sub->topic)) {
...
static void test_add_remove(void **state) {
    ...
    struct jsdrv_context_s * context = initialize();
    expect_list(ex_list_buffer1, sizeof(ex_list_buffer1));
    publish(context, jsdrvp_msg_alloc_value(context, JSDRV_BUFFER_MSG_ACTION_ADD, &jsdrv_union_u8(3)));
    expect_list(ex_list_buffer0, sizeof(ex_list_buffer0));
    publish(context, jsdrvp_msg_alloc_value(context, JSDRV_BUFFER_MSG_ACTION_REMOVE, &jsdrv_union_u8(3)));
    //finalize(context);
}
