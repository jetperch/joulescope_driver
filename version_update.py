...
def _include_h_version(version):
    ...
    regex = re.compile(r'#define JSDRV_VERSION_(MAJOR|MINOR|PATCH)\s')
    path = os.path.join(MYPATH, 'include', 'jsdrv', 'version.h')
    path_tmp = path + '.tmp'
    with open(path, 'rt') as rd:
        with open(path_tmp, 'wt') as wr:
            for line in rd:
                ...
                if m is not None:
                    v = version[k[m[1]]]
                    line = f'#define JSDRV_VERSION_{m[1]} {v}\n'
                wr.write(line)
    os.replace(path_tmp, path)


def _py_version(version):
    path = os.path.join(MYPATH, 'pyjoulescope_driver', 'version.py')
    path_tmp = path + '.tmp'
    with open(path, 'rt') as rd:
        with open(path_tmp, 'wt') as wr:
            for line in rd:
