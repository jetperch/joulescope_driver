...
static bool print_idcode(uint32_t idcode){
	...
	/* ECP5 Parts */
	for(size_t i = 0; i < sizeof(ecp_devices)/sizeof(struct device_id_pair); i++){
	...
	/* NX Parts */
	for(size_t i = 0; i < sizeof(nx_devices)/sizeof(struct device_id_pair); i++){
...
void print_ecp5_status_register(uint32_t status){
	...
	if(verbose){
		...
		switch (bse_error){
			case 0: printf("  BSE Error Code:     No Error (0b000)\n"); break;
			case 1: printf("  BSE Error Code:     ID Error (0b001)\n"); break;
			case 2: printf("  BSE Error Code:     CMD Error - illegal command (0b010)\n"); break;
			case 3: printf("  BSE Error Code:     CRC Error (0b011)\n"); break;
			case 4: printf("  BSE Error Code:     PRMB Error - preamble error (0b100)\n"); break;
			case 5: printf("  BSE Error Code:     ABRT Error - configuration aborted by the user (0b101)\n"); break;
			case 6: printf("  BSE Error Code:     OVFL Error - data overflow error (0b110)\n"); break;
			case 7: printf("  BSE Error Code:     SDM Error - bitstream pass the size of SRAM array (0b111)\n"); break;
		}
...
void print_nx_status_register(uint64_t status){
	...
	if(verbose){
		...
		uint8_t config_target = status & (7 << 1) >> 1;
		switch (config_target){
			case 0: printf("SRAM (0b000)\n"); break;
			case 1: printf("EFUSE Normal (0b001)\n"); break;
			case 2: printf("EFUSE Pseudo (0b010)\n"); break;
			case 3: printf("EFUSE Safe (0b011)\n"); break;
		...
		uint8_t bse_error = (status & (0xf << 24)) >> 24;
		switch (bse_error){
			case 0x0: printf("  BSE Error Code:     No Error (0b000)\n"); break;
			case 0x1: printf("  BSE Error Code:     ID Error (0b001)\n"); break;
			case 0x2: printf("  BSE Error Code:     CMD Error - illegal command (0b010)\n"); break;
			case 0x3: printf("  BSE Error Code:     CRC Error (0b011)\n"); break;
			case 0x4: printf("  BSE Error Code:     PRMB Error - preamble error (0b100)\n"); break;
			case 0x5: printf("  BSE Error Code:     ABRT Error - configuration aborted by the user (0b101)\n"); break;
			case 0x6: printf("  BSE Error Code:     OVFL Error - data overflow error (0b110)\n"); break;
			case 0x7: printf("  BSE Error Code:     SDM Error - bitstream pass the size of SRAM array (0b111)\n"); break;
			case 0x8: printf("  BSE Error Code:     Authentication Error (0b1000)\n"); break;
			case 0x9: printf("  BSE Error Code:     Authentication Setup Error (0b1001)\n"); break;
			case 0xA: printf("  BSE Error Code:     Bitstream Engine Timeout Error (0b1010) \n"); break;
		}
		...
		printf("  Invalid Command:    %s\n",  status & (1 << 30) ? "Yes" : "No" );
		printf("  WDT Busy:           %s\n",  status & (1 << 31) ? "Yes" : "No" );
		printf("  Dry Run DONE:       %s\n",  status & (1ULL << 33) ? "Yes" : "No" );

		uint8_t bse_error1 = (status >> 34) & 0xfUL;
		switch (bse_error1){
			case 0x0: printf("  BSE Error 1 Code: (Previous Bitstream)  No Error (0b000)\n"); break;
			case 0x1: printf("  BSE Error 1 Code: (Previous Bitstream)  ID Error (0b001)\n"); break;
			case 0x2: printf("  BSE Error 1 Code: (Previous Bitstream)  CMD Error - illegal command (0b010)\n"); break;
			case 0x3: printf("  BSE Error 1 Code: (Previous Bitstream)  CRC Error (0b011)\n"); break;
			case 0x4: printf("  BSE Error 1 Code: (Previous Bitstream)  PRMB Error - preamble error (0b100)\n"); break;
			case 0x5: printf("  BSE Error 1 Code: (Previous Bitstream)  ABRT Error - configuration aborted by the user (0b101)\n"); break;
			case 0x6: printf("  BSE Error 1 Code: (Previous Bitstream)  OVFL Error - data overflow error (0b110)\n"); break;
			case 0x7: printf("  BSE Error 1 Code: (Previous Bitstream)  SDM Error - bitstream pass the size of SRAM array (0b111)\n"); break;
			case 0x8: printf("  BSE Error 1 Code: (Previous Bitstream)  Authentication Error (0b1000)\n"); break;
			case 0x9: printf("  BSE Error 1 Code: (Previous Bitstream)  Authentication Setup Error (0b1001)\n"); break;
			case 0xA: printf("  BSE Error 1 Code: (Previous Bitstream)  Bitstream Engine Timeout Error (0b1010) \n"); break;
		}

		printf("  Bypass Mode:        %s\n",  status & (1ULL << 38) ? "Yes" : "No" );
		printf("  Flow Through Mode:  %s\n",  status & (1ULL << 39) ? "Yes" : "No" );
		printf("  SFDP Timeout:       %s\n",  status & (1ULL << 42) ? "Yes" : "No" );
		printf("  Key Destroy Pass:   %s\n",  status & (1ULL << 43) ? "Yes" : "No" );
		printf("  INITN:              %s\n",  status & (1ULL << 44) ? "Yes" : "No" );
		printf("  I3C Parity Error 2: %s\n",  status & (1ULL << 45) ? "Yes" : "No" );
		printf("  Init Bus ID Error:  %s\n",  status & (1ULL << 46) ? "Yes" : "No" );
		printf("  I3C Parity Error 1: %s\n",  status & (1ULL << 47) ? "Yes" : "No" );

		uint8_t auth_mode = (status & (3ULL << 48)) >> 48;
		switch (auth_mode){
			case 0: printf("  Authentication Mode:  No Auth (0b00)\n"); break;
			case 1: printf("  Authentication Mode:  ECDSA (0b01)\n"); break;
			case 2: printf("  Authentication Mode:  HMAC (0b10)\n"); break;
			case 3: printf("  Authentication Mode:  No Auth (0b11)\n"); break;
		}

		printf("  Authentication Done: %s\n",  status & (1ULL << 50) ? "Yes" : "No" );
		printf("  Dry Run Authentication Done: %s\n",  status & (1ULL << 51) ? "Yes" : "No" );
		printf("  JTAG Locked:         %s\n",  status & (1ULL << 52) ? "Yes" : "No" );
		printf("  SSPI Locked:         %s\n",  status & (1ULL << 53) ? "Yes" : "No" );
		printf("  I2C/I3C Locked:      %s\n",  status & (1ULL << 54) ? "Yes" : "No" );
		printf("  PUB Read Lock:       %s\n",  status & (1ULL << 55) ? "Yes" : "No" );
		printf("  PUB Write Lock:      %s\n",  status & (1ULL << 56) ? "Yes" : "No" );
		printf("  FEA Read Lock:       %s\n",  status & (1ULL << 57) ? "Yes" : "No" );
		printf("  FEA Write Lock:      %s\n",  status & (1ULL << 58) ? "Yes" : "No" );
		printf("  AES Read Lock:       %s\n",  status & (1ULL << 59) ? "Yes" : "No" );
		printf("  AES Write Lock:      %s\n",  status & (1ULL << 60) ? "Yes" : "No" );
		printf("  PWD Read Lock:       %s\n",  status & (1ULL << 61) ? "Yes" : "No" );
		printf("  PWD Write Lock:      %s\n",  status & (1ULL << 62) ? "Yes" : "No" );
		printf("  Global Lock:         %s\n",  status & (1ULL << 63) ? "Yes" : "No" );
	}
